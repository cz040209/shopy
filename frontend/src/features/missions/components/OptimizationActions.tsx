...
export default function OptimizationActions({ disabled, onPick }: { disabled: boolean; onPick: (prompt: string) => void }) {
  return <section className={styles.optimizations}><span>RECOMPOSE THIS MISSION</span><div>{actions.map(({ label, prompt, icon: Icon }, index) => <button className={styles[`actionTone${index % 5}`]} type="button" key={label} disabled={disabled} onClick={() => onPick(prompt)}><Icon size={15} />{label}</button>)}</div></section>;
}
