...
export default function BundleBoard({ items, mission, workspace, adding, addingProductId, onAdd, onAddItem, onRemove }: Props) {
  ...
  return <section className={styles.canvas}>
    ...
    {!isSingleRecommendation && <button type="button" className={`${styles.add} shine-button`} disabled={adding || !items.length} onClick={onAdd}><ShoppingBag size={17} />{adding ? "Adding bundle…" : `Add all ${items.length} ${items.length === 1 ? "item" : "items"}`}</button>}
  </section>;
}
