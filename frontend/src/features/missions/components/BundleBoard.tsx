...
export default function BundleBoard({ items, mission, workspace, adding, addingProductId, onAdd, onAddItem, onRemove }: Props) {
  ...
  return <section className={styles.canvas}>
    ...
    {!isSingleRecommendation && plannedCount > 0 && <div className={styles.coverage}><div className={styles.coverageHeading}><span>PLAN COVERAGE</span><strong>{covered.length} of {plannedCount} roles covered</strong></div><div className={styles.coverageChips}>{covered.map((role) => <span className={styles.coveredChip} key={`covered-${role}`}><Check size={13} />{role}</span>)}{missing.map((role) => <span className={styles.missingChip} key={`missing-${role}`}><CircleAlert size={13} />Not found: {role}</span>)}</div>{missing.length > 0 && <p>The available recommendations remain usable, but these requested roles could not be filled from verified catalog matches.</p>}</div>}
