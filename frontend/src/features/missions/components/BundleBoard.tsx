...
import { Check, CircleHelp, ShoppingBag } from "lucide-react";
...
function ProductNode({ item, slot }: { item: Attachment; slot: number }) {
  return <Link href={`/product/${item.product_id}`} className={`${styles.node} ${slotClasses[slot] ?? ""}`}><img src={item.image_url} alt={item.image_alt_text || item.name} /><div><small>{item.category || `Bundle item ${slot + 1}`}</small><strong>{item.name}</strong><b>RM {Number(item.price).toLocaleString("en-MY", { minimumFractionDigits: 0 })}</b></div></Link>;
}

export default function BundleBoard({ items, mission, workspace, adding, onAdd }: Props) {
